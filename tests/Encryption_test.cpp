...
int main() {
    ...
    unsigned char nonce[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4a, 0x00, 0x00, 0x00, 0x00};

    unsigned int initial_counter = 1;
    ...
    unsigned char correct_ciphertext[114] = {
    ...
    for (size_t i = 0; i < 114; ++i) {
        if (cipher[i] != correct_ciphertext[i]) {
            passed = false;
            break;
        }
    }
    ...
    if (passed) {
        std::cout << "ChaChaEncryptionTest : PASSED\n";
        return 0;
    } else {
        std::cout << "ChaChaEncryptionTest : FAILED\n";
        return 1;
    }
}