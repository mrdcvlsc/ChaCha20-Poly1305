...
int main() {
    ...
    uint128 quotient1 = dividen1 / divisor1, correct_quotient1(0,0xfb376b);
    ...
    uint128 AB = A / B, BA = B / A, correct_AB(0,0x6), correct_BA(0,0);

    uint128 C(98789182,87), D(1165125,8787737733);
    uint128 CD = C / D, DC = D / C, correct_CD(0,0x54), correct_DC(0,0);

    uint128 E(18446744073709551615ul,246810121416), F(0,90808664);
    uint128 EF = E / F, FE = F / E, correct_EF(0x2f4c00e9df, 0xf304781481b95104), correct_FE(0,0);
    ...
    if(!failed_cnt) {
        std::cout << "uint128 operator div ALL test PASSED\n";
        return 0;
    }
    else {
        std::cout << "uint128 operator div SOME test FAILED\n";
        return 1;
    }
}